...
export class TraceAnalyser {
  ...
  constructor(traceCache:TraceCache) {
    this.traceCache = traceCache;
  }

  analyse(trace: StructLog[], tx: Transaction) {
    this.trace = trace;
  ...
  buildReturnValues(index: number, step: StructLog) {
    ...
    if (traceHelper.isReturnInstruction(step) || traceHelper.isStopInstruction(step) || traceHelper.isRevertInstruction(step)) {
      this.traceCache.pushStopIndex(index, this.traceCache.currentCall?.call.address ?? null);
    }

    try {
      if (step.gas - step.gasCost <= 0 || step.error === 'OutOfGas') {
        this.traceCache.pushOutOfGasIndex(index, this.traceCache.currentCall?.call.address ?? null);
      }
    } catch (e) {
      console.error(e);
    }
  ...
  buildCalldata(index: number, tx: Transaction, newContext: boolean) {
    let calldata = '';
    if (index === 0) {
      calldata = tx.input;
  ...
  buildMemory(index: number, step: StructLog) {
    if (step.memory) {
      this.traceCache.pushMemoryChanges(index);
    }
  }

  buildStorage(index: number, step: StructLog, context: TraceContext) {
  ...
  buildDepth(index: number, step: StructLog, tx: Transaction, callStack: string[], context: TraceContext) {
    if (traceHelper.isCallInstruction(step) && !traceHelper.isCallToPrecompiledContract(index, this.trace)) {
      ...
      this.buildCalldata(index, tx, true);
      ...
      if (index < this.trace.length) {
        callStack.pop();
        this.traceCache.pushCall(step, index + 1, null, callStack.slice(0), step.error === null || step.invalidDepthChange);
        this.buildCalldata(index, tx, false);
        this.traceCache.pushSteps(index, context.currentCallIndex);
