...
export class InternalCallTree {
  ...
  /**
    ...
    */
  constructor(traceManager: TraceManager, solidityProxy: SolidityProxy) {
    ...
    if (!this.solidityProxy.loaded()) {
      ...
      buildTree(this, 0, '', true, isCreation).then((result: any) => {
        if (result.error) {
          ...
          createReducedTrace(this, this.traceManager.trace!.length - 1)
  ...
  async extractSourceLocation(step: number) {
    try {
      ...
      // return location
      // location : { start, length, file, jump }
      return { start: 0, length: 0, file: "", jump: "" }
    } catch (error) {
...
async function buildTree(tree: InternalCallTree, step: number, scopeId: string, isExternalCall: boolean, isCreation: boolean): Promise<any> {
  ...
  function callDepthChange(step: number, trace: StructLog[]) {
